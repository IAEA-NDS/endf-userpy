      ...
      parameter (nbmax=50000, nnx=450,nnsni=2000,nepmax0=5000)
      parameter (nedim=60,nadim=51,nepdim=200)
      parameter (neu=5,nepu=5,nuu=5)
      parameter (emin=1.0d-5,eps=1.0d-12, one=1.0d0, zero=0.0d0, tol=0.001d0)
      character*66 line
...
!      dimension ep(nedim,nadim,nepdim),f(nedim,nadim,nepdim),ilef(nedim,nadim,nepdim)
       dimension ep(nepmax0),feep(nepmax0),fdev(nepmax0)
      allocatable ep1(:),b1(:,:),ep2(:),b2(:,:),ep0(:)
      data nin/3/,nou/12/
          ...
          if (jp.eq.0.and.lct0.ne.4) then
            ...
            do kk=1,nk
              ...
              if (eprim.eq.zero) then
                ...
                write(*,'(a,1p,e13.6,a,e13.6,a,i2)')' ZAP=',zap,' AWP=',awp,' LCT=',lct
              else
                write(nou,'(a,1p,e13.6,a,e13.6,a,i2,a,e13.6)')' ZAP=',zap,' AWP=',awp,' LCT=',lct,' E_prim=',eprim
                write(*,'(a,1p,e13.6,a,e13.6,a,i2,a,e13.6)')' ZAP=',zap,' AWP=',awp,' LCT=',lct,' E_prim=',eprim
              endif
              if (law.eq.1) then
                ...
                do ie=2, ne
                  ...
                  if (e1.gt.e0) e0=e1
                  h=(e2-e0)/dble(neu-1)
                  write(*,*)' intervalo i: ',ie-1,e1,e2
                  do i=1,neu
                    ei=e0+h*dble(i-1)
                    nepuu=2
                    epu(1)=10.
                    epu(2)=50.
                    call feep_full_law1con(ei,awr,awi,awp,za,zai,zap,lct,lang,lep,lei,e1,nd1,na1,nep1,ep1,b1, &
                                           e2,nd2,na2,nep2,ep2,b2,tol,nepuu,epu,nepmax,ep,feep,fdev,nep)
                    write(*,*)' incident energy i=',i,ei
...
!
!                     checking feep_points_law1con
!
!                    np0=max(nep1-nd1,0)+max(nep2-nd2,0)+max(nepu,0)+5
!                    allocate(ep0(np0))
!                    call initial_epgrid(ei,awr,awi,awp,lct,e1,nd1,nep1,ep1,e2,nd2,nep2,ep2,nepuu,epu,ep0,np0)
!                    call feep_points_law1con(ei,awr,awi,awp,za,zai,zap,lct,lang,lep,lei,e1,nd1,na1,nep1,ep1,b1, &
!                                             e2,nd2,na2,nep2,ep2,b2,tol,np0,ep0,feep,fdev)
!                    write(*,*)'sub-intervalo i=',i
!                    write(*,'(a6,4a20)')' No.',' E',' Ep','f(E,Ep)',' dev(E,Ep)' !DLA
!                    write(nou,'(a6,4a20)')' No.',' E',' Ep','f(E,Ep)',' dev(E,Ep)' !DLA
!                    do j=1,np0
!                      write(*,'(i6,1p4e20.11)')j,ei,ep0(j),feep(j),fdev(j)
!                      write(nou,'(i6,1p4e20.11)')j,ei,ep0(j),feep(j),fdev(j)
!                    enddo
!                    deallocate(ep0)
                    write(*,*)
                    write(nou,*)
