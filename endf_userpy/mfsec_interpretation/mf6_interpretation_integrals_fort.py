"""Fortran-backed MF6 LAW=1 continuum ``E'``-integral wrappers.

Preserved as the equivalence oracle for the backend-agnostic Python
port in :mod:`mf6_interpretation_integrals` (``feep_points_law1con``)
and :mod:`mf6_law1_epintegral` (``integrate_law1_spectrum``).

Two wrappers live here:

- :func:`get_energydist_from_subsec_law1_fort` calls the Fortran
  ``feep_points_law1con`` on the caller-supplied Ep grid.
- :func:`get_energydist_from_subsec_law1_dynamic_mesh_fort` calls the
  Fortran ``feep_full_law1con`` which densifies the Ep mesh
  adaptively; kept for future porting to Python (PR-B in the
...
@pad_outside_energydist_values
def get_energydist_from_subsec_law1_dynamic_mesh_fort(
    ...
    for i in range(result_arr.shape[0]):
        ...
        eouts = ep[:nep]
        idcs2 = find_indices_with_tol(eouts, epu, rtol=1e-8, atol=1e-10)
        result_arr[i,:] = feep[idcs2]

    return result_arr


@pad_outside_energydist_values
def get_energydist_from_subsec_law1_fort(
    endf_dict, mt, subsec_num, energies_in, energies_out, to_lab
):
    sec = endf_dict[6][mt]
    sec['subsection'][subsec_num]['LAW'] == 1
