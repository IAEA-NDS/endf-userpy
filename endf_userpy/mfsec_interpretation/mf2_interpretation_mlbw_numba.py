...
def _low_L_pnt_shf(rho, L):
    ...
    Same expressions as
    :func:`mf2_interpretation_factors.low_L_pnt_shf`, but scalar so
    numba can fuse them into the per-energy loop.
    """
    r2 = rho * rho
    if L == 0:
        return rho, 0.0
        ...
        # L>=6 is rejected at the wrapper boundary; NaN sentinel here
        # would only reach an already-validated call path.
...
def _low_L_phase(rho, L):
    ...
    :func:`mf2_interpretation_factors.low_L_phase` for the closed forms.
    """
    r2 = rho * rho
    if L == 0:
        return rho
