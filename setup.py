...
def _find_gfortran_archive(libname):
    """Path to a static archive (.a) shipped alongside gfortran, or None.

    Used on macOS to bake the gfortran runtime into the extension
    instead of letting it become a runtime dependency on libgfortran.dylib
    that delocate would then pull into the wheel and pin the wheel's
    macOS minimum to whatever the Homebrew gcc bottle targeted.
    """
    if sys.platform != 'darwin':
        return None
    try:
        result = subprocess.check_output(
            ['gfortran', f'-print-file-name=lib{libname}.a'],
            text=True,
        ).strip()
    except (subprocess.CalledProcessError, FileNotFoundError):
        return None
